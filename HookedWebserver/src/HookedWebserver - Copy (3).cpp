...
{
    /* ---------- Cookie — load once from disk ---------- */
    static std::string s_cookie;
    static bool        s_cookieLoaded = false;
    if (!s_cookieLoaded) {
        s_cookieLoaded = true;
        char profile[MAX_PATH] = {0};
        GetEnvironmentVariableA("USERPROFILE", profile, sizeof(profile));
        if (profile[0]) {
            std::string path = std::string(profile) + "\\Documents\\robloxcookie.txt";
            s_cookie = ReadFile(path);
            /* strip trailing whitespace / newlines */
            while (!s_cookie.empty() &&
                   (s_cookie.back() == '\r' || s_cookie.back() == '\n' ||
                    s_cookie.back() == ' '))
                s_cookie.resize(s_cookie.size() - 1);
            Log("FetchUrl: cookie %s",
                s_cookie.empty() ? "not found (public assets only)" : "loaded");
        }
    }
...
static Resp HandleAssetDelivery(const Req& req)
{
    ...
    /* Check 1: local file in www\asset\ next to DLL (mirrors PHP ./{id}) */
    {
        ...
        std::string d = ReadFile(f);
        if (!d.empty()) return AssetServe(d);
    }

    /* Check 2: data\SavedData\{key} with or without extension */
    {
...
/* Resolve an asset's assetTypeId + name via economy.roblox.com (session cache). */
static AssetInfo ResolveAssetInfo(long long assetId)
{
    ...
    char url[160];
    sprintf(url, "https://economy.roblox.com/v2/assets/%lld/details", assetId);
    std::string body; AssetInfo info; info.type = -1;
    if (FetchUrl(url, body)) {
        size_t p = body.find("\"AssetTypeId\"");
        if (p != std::string::npos) {
            p = body.find(':', p);
            if (p != std::string::npos) info.type = atoi(body.c_str() + p + 1);
        }
        info.name = JsonGetString(body, "Name");
    }
    std::lock_guard<std::mutex> lk(g_avMutex);
    g_assetCache[assetId] = info;
    return info;
}
...
static std::string AvatarFetchJson(const std::string& placeId, const std::string& userId)
{
    ...
    std::string json;
    if (!raw.empty()) {
        json = CompileAvatarJson(rig, raw);             /* may block on economy fetches */
    } else {
    ...
    if (STARTS(P,"/Data/Upload.ashx") || STARTS(P,"/data/upload.ashx")) {
        if (M == "POST") {
            ...
            if (!assetId.empty() && !req.body.empty()) {
                std::string dir  = DllPath("data\\SavedData\\");
                EnsureDir(dir);
                /* Save with .rbxl extension so asset delivery can find it.
                 * NOTE: req.body is already gunzipped by ParseHttp when the
                 * upload arrives Content-Encoding: gzip, so we must write it
                 * AS-IS.  Running GzipDecompress here again decompressed the
                 * real (already-plain) .rbxl, returned empty, and saved the
                 * place as 0 bytes -> load 404. */
                std::string savePath = dir + assetId + ".rbxl";
                WriteFile_(savePath, req.body);
                /* Remove any old extensionless file left from previous versions */
                DeleteFileA((dir + assetId).c_str());
                Log("Upload.ashx: saved %s (%d bytes written) -> %s",
                    assetId.c_str(), (int)req.body.size(), savePath.c_str());
