...
namespace RobloxStudioPatcher
{
    ...
    // Cache of the resolved numeric server IP, filled by PatchLocalRccIp() /
    // GetServerEndpoint() so the udp_relay can reuse it without re-resolving.
    static std::string s_resolvedServerIp;

    // Pull the "-port <n>" connect port off the command line (the port the
    // client uses to reach the host; tunnelled to the host's RbxTransport port).
    static int ParseConnectPort()
    {
    ...
    bool GetServerEndpoint(std::string& ipOut, int& portOut)
    {
        if (s_resolvedServerIp.empty())
        {
            std::string host = GetServerArg();
            if (!host.empty()) s_resolvedServerIp = ResolveToIpv4(host);
        }
        int port = ParseConnectPort();
        if (s_resolvedServerIp.empty() || port == 0) return false;
        ipOut   = s_resolvedServerIp;
        portOut = port;
        return true;
    }
    ...
    bool PatchLocalRccIp()
    {
        ...
        s_resolvedServerIp = ip;   // cache for udp_relay's identity announcer
        if (ip != host)
