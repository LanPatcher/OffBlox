...
namespace RobloxStudioPatcher
{
    ...
    // Resolved connect endpoint for the client: the numeric IPv4 the "-server"
    // host resolved to (e.g. "147.185.221.22") and the "-port" connect port
    // (e.g. 50064). This is the exact destination the client's QUIC/RbxTransport
    // traffic is tunnelled to, so the udp_relay can aim its identity datagram at
    // the same host:port. Returns false if -server/-port aren't both known.
    // Safe to call after PatchLocalRccIp(); it resolves on first use otherwise.
