...
namespace RobloxStudioPatcher
{
    ...
    static LRESULT CALLBACK ViewportExpanderWndProc(HWND h, UINT msg,
                                                    WPARAM wp, LPARAM lp)
    {
        // In play / loading mode the game renders fullscreen in this engine
        // widget and the engine sets the cursor to NULL (hides the mouse icon)
        // for gameplay. That happens via WM_SETCURSOR on this window, so a
        // ShowCursor counter correction can't win. Intercept it and force a
        // visible arrow over the viewport client area so the client always has
        // a usable pointer. (In edit mode the same override is harmless - the
        // chrome is hidden, so there are no tool cursors to preserve.)
    ...
    static int HideChromeImpl(HWND mainHwnd)
    {
        ...
        // There can be SEVERAL QEngineWidgets in the tree:
        //   * the editor/play DOCUMENT viewport, nested inside a Qtitan
        //     DockDocument* container (this is the one we want);
        //   * the Start Page / Home render - a FULL-WINDOW engine widget
        //     parented directly to the main window (it has QLabel/QFrame
        //     children). This one is the LARGEST, so a pure largest-area rule
        //     selects it - and expanding/focusing it shows the wrong surface
        //     and makes the engine hide the OS cursor;
        //   * the CoreGui TopBar render and small docked previews.
        // So prefer the engine widget that sits under a DockDocument*
        // container; only fall back to largest-overall before a place is open
        // (when the document viewport doesn't exist yet).
        ...
        int  bestArea = -1;
        // First choice: largest engine widget under a DockDocument container.
        for (const WinInfo& w : wins)
        {
            if (!isEngine(w) || !hasDocAncestor(w.hwnd)) continue;
            if (!viewport || w.area > bestArea) { viewport = w.hwnd; bestArea = w.area; }
        }
        // Fallback: largest engine widget anywhere (start-page phase, no doc yet).
        if (!viewport)
        {
            for (const WinInfo& w : wins)
            {
                if (!isEngine(w)) continue;
                if (!viewport || w.area > bestArea) { viewport = w.hwnd; bestArea = w.area; }
            }
        }
        if (!viewport)
        {
            // Last resort: largest visible non-chrome window (only used if no
            // engine widget exists in the tree at all).
            bestArea = 0;
            for (const WinInfo& w : wins)
            {
        ...
        // Snapshot the OS cursor show-count. Our SetWindowPos / hide calls
        // below run the engine widget's window handlers synchronously on this
        // (GUI) thread, and those handlers decrement the ShowCursor counter.
        // Across the steady ~600ms re-hide cadence that drift accumulates and
        // leaves the cursor stuck (and increasingly) hidden. After the pass we
        // restore ONLY the amount WE removed - never bumping above the starting
        // value - so a game that legitimately hides the cursor is untouched.
        ...
        const int cursorBefore = PeekCursorCount();
        ...
        // Undo any cursor-hide WE caused this pass (see snapshot above).
        int cursorAfter = PeekCursorCount();
        while (cursorAfter < cursorBefore)
