...
namespace OffBloxLauncher
{
    ...
    public class MainForm : Form
    {
        ...
        // Derives a stable numeric ID from a username. Mirrors HookedWebserver's
        // UserIdFromUsername() exactly (FNV-1a 32-bit hash, mapped into the
        // 8-digit range [10000000, 99999999]) so the CreatorId we hand to Studio
        // always matches the userId the webserver computes locally for the same
        // username. Same username -> same ID every time; no Roblox account or
        // service involved, it's just a deterministic local placeholder ID.
        private static string UserIdFromUsername(string name)
        {
            ...
            {
                hash ^= b;
                hash *= 16777619u; // FNV prime; uint multiply wraps mod 2^32, same as the C++ uint32_t
            }
            uint id = 10000000u + (hash % 90000000u);
        ...
        private void LoadWorlds()
        {
            ...
            allWorlds.Clear();
            // Same identity the webserver will authenticate the local player as,
            // so the creator id baked into -creatorId always lines up with the
            // userId HookedWebserver derives for this username.
            string creatorId = UserIdFromUsername(OneLine(txtUser != null ? txtUser.Text : ""));
            try
            {
