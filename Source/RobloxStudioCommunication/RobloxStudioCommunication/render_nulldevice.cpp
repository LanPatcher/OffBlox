...
namespace RobloxStudioPatcher
{
    ...
    // Build:  mov ecx, kForcedMode ; jmp factory   (original, known-good tail-jump)
    // The original `call` pushes the return address (0x1e101a3); the factory's own
    // `ret` therefore returns straight back into the try-loop. We touch no other
    // register, so rdx/r8 (the factory's other args) are preserved. (The earlier
    // return-capturing variant that also logged the result was reverted - it
    // regressed engine startup.)
    ...
    void StartForceNullDevice()
    {
        ...
        // Forcing mode 9 (NoGraphics) is now OPT-IN, not the default. Reason:
        // forcing it crashed the engine under Wine right after RenderView
        // creation (the forced null-graphics view is dereferenced/written by the
        // post-create path). By default we let the engine build its normal
        // backend (Direct3D11/OpenGL via Wine), which works wherever a display
        // exists (incl. WSLg) and on Windows; 3D work is still neutered by
        // render_disable. To force the true zero-display NoGraphics path (for a
        // genuinely headless host), create an empty nulldevice_on.txt next to
        // the DLL - that path still needs the Wine crash resolved first.
        {
            std::wstring on = GetDllDirectory() + L"nulldevice_on.txt";
            if (GetFileAttributesW(on.c_str()) == INVALID_FILE_ATTRIBUTES)
            {
                LogF(L"[null_device] NOT forcing NoGraphics (default; engine uses "
                     L"its normal backend). Create nulldevice_on.txt to force the "
                     L"zero-display path.\n");
                return;
            }
        }
