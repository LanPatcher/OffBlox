...
namespace RobloxStudioPatcher
{
    ...
    enum {
        ...
        WV_get_CookieManager = 66,             // get_CookieManager (verified on this
                                               // build via runtime vtable probe; the
                                               // documented ICoreWebView2_2 value 59 is
                                               // wrong here - the webview's base vtable
                                               // has extra slots before _2).
        // ICoreWebView2CookieManager / ICoreWebView2CookieList
        CM_GetCookies = 5, CL_get_Count = 3,
    ...
    static void* g_cmVtbl[24];      // ICoreWebView2CookieManager
    static void* g_clVtbl[24];      // ICoreWebView2CookieList

    static ComObj g_env, g_ctrl, g_wv, g_settings, g_nsa, g_nca, g_hdr;
    static ComObj g_cookieMgr, g_cookieList;
    ...
    // ---- ICoreWebView2CookieManager / CookieList (login cookie-clear) ---
    // Studio clears cookies before navigating to the login URL. Our fake
    // CookieManager reports ZERO cookies so the async clear completes at once and
    // login proceeds. Without firing the completion handler, Studio hangs forever
    // at "clearAllCookiesAndRunCallbackHelper calling GetCookies".
    static HRESULT STDMETHODCALLTYPE CL_GetCount(void* /*self*/, UINT* count)
    { if (count) *count = 0; return S_OK; }
    static HRESULT STDMETHODCALLTYPE CM_DoGetCookies(void* /*self*/, LPCWSTR /*uri*/, void* handler)
    {
        LogF(L"[webview_skip] CookieManager::GetCookies -> 0 cookies (clear completes)\n");
        if (handler) CallHandlerInvoke3(handler, (void*)(intptr_t)S_OK, &g_cookieList);
        return S_OK;
    }
    static HRESULT STDMETHODCALLTYPE WV_GetCookieMgr(void* /*self*/, void** out)
    { LogF(L"[webview_skip] get_CookieManager -> fake manager\n"); if (out) *out = &g_cookieMgr; return S_OK; }
    ...
    // True only for the OAuth login navigation (…/oauth/…/authorize…). Cookie-
    // clear and post-login content navigations must NOT get the auth redirect.
    static bool IsAuthorizeUrl(LPCWSTR url)
    {
        if (!url) return false;
        std::wstring u(url);
        for (wchar_t& c : u) if (c >= L'A' && c <= L'Z') c = (wchar_t)(c - L'A' + L'a');
        return u.find(L"authorize") != std::wstring::npos
            || u.find(L"/oauth/")  != std::wstring::npos;
    }

    static HRESULT STDMETHODCALLTYPE WV_DoNavigate(void* /*self*/, LPCWSTR url)
    {
        // Non-login navigation (cookie-clear, or a content page under Wine): report
        // it as-is and do NOT inject the auth code, so nothing is hijacked.
        if (!IsAuthorizeUrl(url))
        {
            g_redirectUrl = url ? url : L"about:blank";
            LogF(L"[webview_skip] Navigate (non-login) passthrough: %s\n",
                 g_redirectUrl.c_str());
    ...
    static void BuildVtables()
    {
        ...
        for (int i = 0; i < (int)(sizeof(g_cmVtbl) / sizeof(void*)); ++i) g_cmVtbl[i] = (void*)&Gen_Ok;
        for (int i = 0; i < (int)(sizeof(g_clVtbl) / sizeof(void*)); ++i) g_clVtbl[i] = (void*)&Gen_Ok;

        void* vts[] = { g_envVtbl, g_ctrlVtbl, g_wvVtbl, g_setVtbl, g_nsaVtbl, g_ncaVtbl, g_hdrVtbl, g_cmVtbl, g_clVtbl };
        for (void* vp : vts)
        {
            void** v = (void**)vp;
        ...
        g_wvVtbl[WV_get_CookieManager]                   = (void*)&WV_GetCookieMgr;
        ...
        g_cmVtbl[CM_GetCookies] = (void*)&CM_DoGetCookies;
        g_clVtbl[CL_get_Count]  = (void*)&CL_GetCount;
        ...
        g_cookieMgr.vtbl = g_cmVtbl; g_cookieList.vtbl = g_clVtbl;
    }
    ...
    {
        // Under Wine there is NO WebView2 runtime, so handing ANY call to the real
        // loader fails (E_FAIL). Worse, before the real login dialog Studio spins
        // up a throwaway WebView2 to clear cookies (clearAllCookiesAndRunCallback);
        // that consumed the one-shot shim below and left the actual login call to
        // fall through to the (missing) runtime -> "Embedded Web Browser fail to
        // load". So under Wine we shim EVERY environment creation. WV_DoNavigate
        // only injects the OAuth redirect for the real `authorize` navigation, so
        // the cookie-clear and any post-login content views complete harmlessly
        // instead of being hijacked.
        const bool wine = RunningUnderWine();

        if (!wine && InterlockedCompareExchange(&g_loginShimmed, 1, 0) != 0)
        {
            // Windows, after the login dialog: these are real content browsers
            // (home/start page) - hand them to the real runtime untouched.
        ...
        LogF(L"[webview_skip] CreateCoreWebView2EnvironmentWithOptions shimmed (%hs)\n",
             wine ? "wine: all envs" : "login");
        BuildVtables();
        EnsurePump();
